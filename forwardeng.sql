...
-- Sun 03 Oct 2021 01:01:34 PM -05
...
CREATE TABLE IF NOT EXISTS `promoterdb`.`supervisor` (
  ...
  `supervisorchainid` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`supervisorinfoid`),
  INDEX `fk_supervisor_chain1_idx` (`supervisorchainid` ASC) VISIBLE,
  CONSTRAINT `fk_supervisor_personal_info1`
    FOREIGN KEY (`supervisorinfoid`)
    REFERENCES `promoterdb`.`personal_info` (`infoid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_supervisor_chain1`
    FOREIGN KEY (`supervisorchainid`)
    REFERENCES `promoterdb`.`chain` (`chainid`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
